...
import { ref } from 'vue';
import { 
  X, Download, Upload, Droplets, Settings, Globe, 
  Database, Type, HelpCircle, Clock, ShieldCheck, 
  Monitor, Briefcase, Activity, FileJson, Server
  ...
  if (localSettings.value.waterReminderEnabled) {
    ...
    if (!granted) {
      sToast.fire({
        ...
        text: 'Permita as notificações para receber os lembretes.',
        timer: 5000
      });
...
const handleTestNotification = async () => {
  sendNotification('Teste TASK 💧', 'Sua notificação está funcionando!');
  sToast.fire({ icon: 'success', title: 'Teste Disparado! 💧', toast: true, position: 'bottom-end' });
};
...
<template>
  <div class="fixed inset-0 bg-slate-950/60 backdrop-blur-sm z-50 flex items-center justify-center animate-[fadeIn_0.3s_ease-out]" @click.self="emit('close')">
    <section class="glass-panel w-[95%] max-w-4xl p-0 animate-scaleIn h-[90vh] md:h-[600px] flex flex-col overflow-hidden shadow-2xl border-indigo-500/10">
