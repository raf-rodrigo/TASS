          ...
          <div class="flex items-center justify-between px-6 md:px-10 py-3 border-b border-app-border-light shrink-0">
            <div class="flex items-center gap-2.5">
              <template v-if="showDrivePicker">
                <button @click="showDrivePicker = false" class="p-1 hover:bg-app-surface rounded-lg transition-colors mr-2">
                  <ArrowLeft class="w-4 h-4 text-app-muted" />
                </button>
                <Cloud class="w-3.5 h-3.5 shrink-0 text-indigo-500" />
                <div>
                  <p class="text-[11px] font-black text-app-main leading-none uppercase tracking-wider">Google Drive</p>
                  <p class="text-[9px] text-app-muted font-medium mt-0.5">Selecione uma imagem para importar.</p>
                </div>
              </template>
              <template v-else>
