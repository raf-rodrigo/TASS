...
const runRebuildPipeline = async (type) => {
  ...
  const target = type === 'dev' 
    ? settingsStore.activeBranchDev 
    : (type === 'test' ? settingsStore.activeBranchTest : settingsStore.activeBranchHml);
...
const executeDeleteBranch = async () => {
  ...
  const protectedBranches = [settingsStore.activeBranchMaster, settingsStore.activeBranchHml, settingsStore.activeBranchTest, settingsStore.activeBranchDev];
    ...
    <template #default>
      ...
      <div v-else-if="activeTab === 'merges'" :key="'merges'" class="space-y-4 animate-fadeIn flex-1 flex flex-col min-h-0 lg:overflow-hidden overflow-y-auto h-full w-full pb-8">
        ...
        <ActionPanel
          ...
          :branchTest="settingsStore.activeBranchTest"
